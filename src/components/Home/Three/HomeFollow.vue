<template>
	...
	<div class="hot">
		...
		<div v-else class="mkback" style="height: 700px;line-height: 700px;margin-top: 10px;text-align: center;">
			<div>
				<span class="text_14 text_color1">暂无关注动态</span>
			</div>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 	if (this.busy) {
			// 		setTimeout(() => {
			// 			for (var i = 0, j = 2; i < j && this.count < this.postdata.length; i++) {
			// 				this.zcpostdata.push(this.postdata[this.count])
			// 				this.count++
			// 			}
			// 			this.busy = false
			// 		});
			...
			//获取指定动态信息
			async SePostData() {
				const res = await this.$http.post('Index/SeUserAddedPost', {
					userid: this.id
				})
